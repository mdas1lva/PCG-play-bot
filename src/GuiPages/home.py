...
from PyQt6.QtCore import QUrl, pyqtSlot, QMetaObject, Qt, Q_ARG, QRect, QObject
...
class Bridge(QObject):
    ...
    Isolated from QWebEngineView to avoid property warnings.
    """
    def __init__(self, parent_callbacks):
        super().__init__()
        self.callbacks = parent_callbacks

    @pyqtSlot(str)
    def change_bot_status(self, new_status):
        self.callbacks["change_bot_status"](new_status)

    @pyqtSlot()
    def request_open_config(self):
        self.callbacks["request_open_config"]()

    @pyqtSlot()
    def request_twitch_login(self):
        self.callbacks["request_twitch_login"]()

    @pyqtSlot()
    def request_twitch_logout(self):
        self.callbacks["request_twitch_logout"]()

    @pyqtSlot(str)
    def handle_open_link(self, link):
        self.callbacks["handle_open_link"](link)
...
class HomePage(QWebEngineView):
    ...
    def __init__(self,
        ...
        # Store callbacks for the bridge
        callbacks = {
            "change_bot_status": change_bot_status,
            "request_open_config": request_open_config,
            "request_twitch_login": request_twitch_login,
            "request_twitch_logout": request_twitch_logout,
            "handle_open_link": web_open
        }

        self._channel = QWebChannel()
        self._bridge = Bridge(callbacks)
        ...
        self.setGeometry(QRect(0, 0, 470, 650))
    ...
    def _invoke_update_store(self, command):
        """Channel update store to main thread."""
